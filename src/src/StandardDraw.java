 ...
 *  Getting started.
 *  To use this class, you must have {@code StandardDraw.class} in your
 ...
 *  @author Kevin Wayne
 */
public class StandardDraw implements ActionListener, MouseListener, MouseMotionListener, KeyListener {
    ...
    private static StandardDraw std = new StandardDraw();
    ...
    private static void init() {
        ...
        onscreenImage  = new BufferedImage(getWidth(), getHeight(), BufferedImage.TYPE_INT_ARGB);
    ...
    ***************************************************************************/
    ...
    private static double  scaleX(double x) { return getWidth()  * (x - xmin) / (xmax - xmin); }
    private static double  scaleY(double y) { return getHeight() * (ymax - y) / (ymax - ymin); }
    private static double factorX(double w) { return w * getWidth()  / Math.abs(xmax - xmin);  }
    private static double factorY(double h) { return h * getHeight() / Math.abs(ymax - ymin);  }
    private static double   userX(double x) { return xmin + x * (xmax - xmin) / getWidth();    }
    private static double   userY(double y) { return ymax - y * (ymax - ymin) / getHeight();   }
...
*/
     ...
     * @throws IllegalArgumentException if the image filename is invalid
     */
    public static void picture(double x, double y, double offsetx, double offset, String filename, double degrees) {
        // BufferedImage image = getImage(filename);
        Image image = getImage(filename);
        double xs = scaleX(x);
        double ys = scaleY(y);
    ...
    ***************************************************************************/
    ...
    public static void save(String filename) {
        ...
        // reference: http://archives.java.sun.com/cgi-bin/wa?A2=ind0404&L=java2d-interest&D=0&P=2727
            ...
            newRaster = raster.createWritableChild(0, 0, getWidth(), getHeight(), 0, 0, new int[] {0, 1, 2});
    ...
    public void actionPerformed(ActionEvent e) {
        FileDialog chooser = new FileDialog(StandardDraw.frame, "Use a .png or .jpg extension", FileDialog.SAVE);
        chooser.setVisible(true);
        String filename = chooser.getFile();
        if (filename != null) {
            StandardDraw.save(chooser.getDirectory() + File.separator + chooser.getFile());
        }
    }
    ...
    ***************************************************************************/
    ...
    public void mousePressed(MouseEvent e) {
        synchronized (mouseLock) {
            mouseX = StandardDraw.userX(e.getX());
            mouseY = StandardDraw.userY(e.getY());
            isMousePressed = true;
        }
    }
    ...
    *  as an argument. It can handle action keys
     ...
     */
    public static void main(String[] args) {
        StandardDraw.square(0.2, 0.8, 0.1);
        StandardDraw.filledSquare(0.8, 0.8, 0.2);
        StandardDraw.circle(0.8, 0.2, 0.2);

        StandardDraw.setPenColor(StandardDraw.BOOK_RED);
        StandardDraw.setPenRadius(0.02);
        StandardDraw.arc(0.8, 0.2, 0.1, 200, 45);

        // draw a blue diamond
        StandardDraw.setPenRadius();
        StandardDraw.setPenColor(StandardDraw.BOOK_BLUE);
        double[] x = { 0.1, 0.2, 0.3, 0.2 };
        double[] y = { 0.2, 0.3, 0.2, 0.1 };
        StandardDraw.filledPolygon(x, y);
        ...
        StandardDraw.text(0.2, 0.5, "black text");
        StandardDraw.setPenColor(StandardDraw.WHITE);
        StandardDraw.text(0.8, 0.8, "white text");
    }
